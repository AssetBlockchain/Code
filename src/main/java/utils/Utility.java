...
private static void addUser() {
	...
	JsonNode participants = mapper.readValue(Participants.participantsInfo, JsonNode.class);
	
	/*ArrayNode  regulators=mapper.readValue(participants.get("regulators").asText(),ArrayNode.class);
	ArrayNode  manufacturers=mapper.readValue(participants.get("manufacturers").asText(),ArrayNode.class);
	ArrayNode  dealerships=mapper.readValue(participants.get("dealerships").asText(),ArrayNode.class);
	ArrayNode  lease_companies=mapper.readValue(participants.get("lease_companies").asText(),ArrayNode.class);
	ArrayNode  leasees=mapper.readValue(participants.get("leasees").asText(),ArrayNode.class);
	ArrayNode  scrap_merchants=mapper.readValue(participants.get("scrap_merchants").asText(),ArrayNode.class);
	*/
	Iterator<String> userFields = participants.fieldNames();
	ArrayList<User> users=new ArrayList<User>();
	
	while(userFields.hasNext())
	{
		String type=userFields.next();
		ArrayNode  role=mapper.readValue(participants.get(type).asText(),ArrayNode.class);
		for (JsonNode jsonNode : role) {
			User user=new User();
			user.setIdentity(jsonNode.get("identity").asText());
